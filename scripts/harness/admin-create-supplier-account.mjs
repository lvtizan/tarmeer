...
try {
  ...
  check('listing report excludes zero-product accounts and uses first product time', /sp\.first_product_at IS NOT NULL/.test(supplierControllerSource) && /DATE\(sp\.first_product_at\)/.test(supplierControllerSource) && !/sp\.is_published = 1 AND sp\.first_product_at/.test(supplierControllerSource), 'listing report must use first_product_at without publish gating');
  const partnerPublishSource = readFileSync(path.join(ROOT, 'server/dist/lib/partnerPublishService.js'), 'utf8');
  check('all product creation paths record first product time', /first_product_at = COALESCE\(first_product_at, NOW\(\)\)/.test(supplierControllerSource) && /first_product_at = COALESCE\(first_product_at, NOW\(\)\)/.test(readFileSync(path.join(ROOT, 'server/dist/controllers/supplierProductController.js'), 'utf8')) && /first_product_at=COALESCE\(first_product_at,NOW\(\)\)/.test(partnerPublishSource), 'first product write path missing');
  ...
  check('supplier report renders a fixed five-per-day native bar chart', /function DailyListingBarChart/.test(supplierReportSource) && /DAILY_LISTING_CAPACITY = 5/.test(supplierReportSource) && /rotate\(-62deg\)/.test(supplierReportSource), 'daily listing bar chart must use a fixed capacity and steep date labels');
