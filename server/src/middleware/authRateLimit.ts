...
const lockedAccounts = new Map<string, { lockUntil: number; reason: string }>();
 ...
 * 检查账户是否被锁定
 */
function isAccountLocked(identifier: string): { locked: boolean; reason?: string; remainingTime?: number } {
 ...
 * 账户锁定检查中间件
 */
export function checkAccountLock(req: Request, res: Response, next: NextFunction): void {
  // 从请求中获取标识符（email或IP）
 ...
 * 记录认证失败的中间件
 */
export function recordAuthFailure(req: Request, res: Response, next: NextFunction): void {
  const identifier = req.body?.email || req.ip;
 ...
 * 记录认证成功的中间件
 */
export function recordAuthSuccess(req: Request, res: Response, next: NextFunction): void {
  const identifier = req.body?.email || req.ip;
