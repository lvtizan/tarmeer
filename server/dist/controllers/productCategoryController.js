...
async function deleteProductCategory(req, res) {
  try {
    const value = decodeURIComponent(req.params.value);
    await pool.execute('UPDATE product_categories SET parent_value = NULL WHERE parent_value = ?', [value]);
    await pool.execute('DELETE FROM product_categories WHERE value = ?', [value]);
...
// GET /product-categories（公开）— 仅启用项，供前台产品筛选/展示用
