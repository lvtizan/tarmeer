...
async function hasColumn(table, column) {
    const [rows] = await database_1.default.execute(`SELECT COUNT(*) as count
 ...
 */
async function listHomeowners(req, res) {
    try {
        ...
        const [rows] = await database_1.default.execute(`SELECT hp.*, u.email, u.full_name as user_name, u.avatar_url, u.created_at as user_created_at,
       ...
       LIMIT ${Number(limit)} OFFSET ${Number(offset)}`, homeownerParams);
...
async function assignDesigner(req, res) {
    try {
        ...
        const [designerRows] = await database_1.default.execute('SELECT id, full_name FROM designers WHERE id = ? AND (status = \'approved\' OR is_approved = 1)', [designer_id]);
        if (designerRows.length === 0) {
            return res.status(404).json({ error: 'Approved designer not found.' });
        }
        await database_1.default.execute('UPDATE homeowner_profiles SET assigned_designer_id = ?, assigned_at = NOW() WHERE id = ?', [designer_id, id]);
        res.json({
            message: 'Designer assigned successfully.',
            designer_name: designerRows[0].full_name,
        });
    }
    catch (error) {
...
async function listCompanies(req, res) {
    try {
        ...
        const VALID_COUNTRIES = new Set(['ae', 'vn', 'sa']);
        if (country && VALID_COUNTRIES.has(country)) {
            whereClause += ' AND cp.country = ?';
            params.push(country);
        }
        const [countRows] = await database_1.default.execute(`SELECT COUNT(*) as total FROM company_profiles cp JOIN users u ON u.id = cp.user_id ${whereClause}`, params);
        const total = countRows[0].total;
...
async function approveCompany(req, res) {
    try {
        ...
        await database_1.default.execute(`UPDATE company_profiles SET status = 'approved', reviewed_by = ?, reviewed_at = NOW() WHERE id = ?`, [adminId, id]);
        // 公司审核通过后，自动发布所有项目（审批在公司层面，不需逐个审批项目）
        await database_1.default.execute(`UPDATE projects SET status = 'published' WHERE company_profile_id = ? AND status IN ('pending', 'draft')`, [id]);
        // Fetch profile name for logging
        const [profileRows] = await database_1.default.execute('SELECT company_name FROM company_profiles WHERE id = ? LIMIT 1', [id]);
...
async function rejectCompany(req, res) {
    try {
        ...
        await database_1.default.execute(`UPDATE company_profiles SET status = 'rejected', admin_notes = ?, reviewed_by = ?, reviewed_at = NOW() WHERE id = ?`, [reason || null, adminId, id]);
 ...
 */
async function bulkUnapproveCompanies(req, res) {
    try {
        const { ids } = req.body;
        ...
        await database_1.default.execute(`UPDATE company_profiles SET status = 'pending', home_display_order = 0, list_display_order = 0 WHERE id IN (${placeholders}) AND status = 'approved'`, ids);
        // Unpublish their projects too
        await database_1.default.execute(`UPDATE projects SET status = 'draft' WHERE company_profile_id IN (${placeholders}) AND status = 'published'`, ids);
...
async function updateCompanyDisplayOrder(req, res) {
    try {
        ...
        const [currentRows] = await database_1.default.execute('SELECT COALESCE(display_order, 0) AS display_order FROM company_profiles WHERE id = ? LIMIT 1', [profileId]);
        ...
        if (displayOrder !== Number(current.display_order || 0) && displayOrder > 0) {
         ...
         WHERE COALESCE(display_order, 0) = ?
            OR COALESCE(home_display_order, 0) = ?
            OR COALESCE(list_display_order, 0) = ?
         LIMIT 1`, [displayOrder, displayOrder, displayOrder, profileId, displayOrder, displayOrder, displayOrder]);
        ...
        await database_1.default.execute('UPDATE company_profiles SET display_order = ? WHERE id = ?', [displayOrder, id]);
...
async function updateCompanySingleOrderField(profileId, orderValue, field) {
    const [currentRows] = await database_1.default.execute(`SELECT COALESCE(${field}, 0) AS current_value FROM company_profiles WHERE id = ? LIMIT 1`, [profileId]);
    const current = currentRows[0];
    if (!current)
        throw new Error(`${field}#NOT_FOUND#${profileId}`);
    if (orderValue !== Number(current.current_value || 0) && orderValue > 0) {
       ...
       LIMIT 1`, [orderValue, profileId, orderValue]);
        if (conflicts.length > 0) {
            const conflict = conflicts[0];
            throw new Error(`${field}#CONFLICT#${orderValue}#${conflict.name}`);
        }
    }
    await database_1.default.execute(`UPDATE company_profiles SET ${field} = ? WHERE id = ?`, [orderValue, profileId]);
}
...
async function deleteCompanyProfile(req, res) {
    try {
        ...
        const [rows] = await database_1.default.execute('SELECT id, company_name, user_id, deleted_at FROM company_profiles WHERE id = ? LIMIT 1', [id]);
        const profile = rows[0];
        if (!profile)
            return res.status(404).json({ error: 'Company profile not found.' });
        const userId = profile.user_id;
        // Log before deletion (audit trail preserved)
        await (0, adminController_1.logActivity)(adminId, 'delete_company_profile', 'company_profile', id, {
            company_name: profile.company_name,
            user_id: userId,
            reason,
        });
        // Cascading hard delete — child tables first to respect FK constraints
        await database_1.default.execute('DELETE FROM projects WHERE company_profile_id = ?', [id]);
        await database_1.default.execute('DELETE FROM articles WHERE company_profile_id = ?', [id]);
        await database_1.default.execute('DELETE FROM company_profiles WHERE id = ?', [id]);
        if (userId) {
 ...
 */
async function restoreCompanyProfile(req, res) {
    try {
        const id = Number(req.params.id);
        const adminId = req.admin?.id;
        if (!Number.isFinite(id) || id <= 0)
        ...
        if (!profile.deleted_at)
            return res.status(400).json({ error: 'Company profile is not deleted.' });
        await database_1.default.execute(`UPDATE company_profiles
       SET deleted_at = NULL, deleted_by_admin_id = NULL, delete_reason = NULL
       WHERE id = ?`, [id]);
        await (0, adminController_1.logActivity)(adminId, 'restore_company_profile', 'company_profile', id, {
            company_name: profile.company_name,
