...
exports.updateCompanyDisplayOrder = updateCompanyDisplayOrder;
exports.updateCompanyHomeDisplayOrder = updateCompanyHomeDisplayOrder;
exports.updateCompanyListDisplayOrder = updateCompanyListDisplayOrder;
exports.getHomeOrderCount = getHomeOrderCount;
exports.listCompanyApplications = listCompanyApplications;
exports.reviewCompanyApplication = reviewCompanyApplication;
exports.bindUserToCompany = bindUserToCompany;
exports.getScrapedCompany = getScrapedCompany;
exports.editScrapedCompany = editScrapedCompany;
exports.getCompanyProfile = getCompanyProfile;
exports.setCompanyCoverImage = setCompanyCoverImage;
exports.setDirectoryCompanyCoverImage = setDirectoryCompanyCoverImage;
exports.editCompanyProfile = editCompanyProfile;
exports.unbindCompany = unbindCompany;
exports.getCompanyProfileFullDetail = getCompanyProfileFullDetail;
exports.getCompanyFullDetail = getCompanyFullDetail;
exports.deleteDirectoryPortfolioImage = deleteDirectoryPortfolioImage;
exports.getAdminProject = getAdminProject;
exports.updateAdminProject = updateAdminProject;
...
exports.getWeightConfigList = getWeightConfigList;
exports.updateWeightConfig = updateWeightConfig;
exports.triggerWeightRecalculation = triggerWeightRecalculation;
exports.adminCrmProvisionCompany = adminCrmProvisionCompany;
...
const PUBLIC_UPLOADS_DIR = path_1.default.join(__dirname, '..', '..', 'public', 'uploads');
...
function toStringArray(value) {
    ...
    const collect = (node) => {
        ...
        if (typeof node === 'string')
            return node ? [normalizeLegacyImageUrl(node)] : [];
...
function normalizeUploadsPath(url) {
    let value = url.trim();
    if (!value)
        return '';
    try {
...
function collapseLegacyProjectPath(uploadPath) {
    return uploadPath.replace(/^\/uploads\/projects\/(\d+)\/\d+\/(\d{4})\/(\d{2})\/([^/?#]+)$/i, '/uploads/projects/$1/$2/$3/$4');
}
function buildProjectImageCandidates(rawUrl) {
...
function uploadPathExists(uploadPath) {
    if (!uploadPath.startsWith('/uploads/'))
        return false;
    const localPath = path_1.default.join(PUBLIC_UPLOADS_DIR, uploadPath.replace(/^\/uploads\//, ''));
    return fs_1.default.existsSync(localPath);
}
function resolveExistingProjectImage(url) {
...
function sanitizeProjectImages(imagesField) {
 ...
 */
function extractAllProjectImageUrls(imagesField) {
    const rawImages = toStringArray(imagesField);
    const seen = new Set();
...
function toCategorizedPortfolioItems(value) {
    const parsed = parseJsonField(value);
    if (!parsed)
        return [];
    const items = [];
...
async function listCompanies(req, res) {
    try {
        ...
        if (claimed === 'claimed') {
            where += ' AND c.owner_user_id IS NOT NULL';
        }
        if (claimed === 'unclaimed') {
            where += ' AND c.owner_user_id IS NULL';
        }
        if (search) {
...
async function updateCompanyDisplayOrder(req, res) {
    try {
        const { companyId } = req.params;
        const raw = req.body?.display_order;
        const displayOrder = Number.isFinite(Number(raw)) ? Math.max(0, Number(raw)) : 0;
        const companyIdNum = Number(companyId);
        const [currentRows] = await database_1.default.execute('SELECT COALESCE(display_order, 0) AS display_order FROM uae_companies WHERE id = ? LIMIT 1', [companyIdNum]);
        const current = currentRows[0];
        if (!current)
            return res.status(404).json({ error: 'Company not found.' });
        // No-op update should always pass.
        if (displayOrder !== Number(current.display_order || 0) && displayOrder > 0) {
            const [conflicts] = await database_1.default.execute(`SELECT 'company_profiles' AS source, id
         ...
         WHERE (
           ...
           AND id <> ?
         LIMIT 1`, [displayOrder, displayOrder, displayOrder, displayOrder, companyIdNum]);
        ...
        await database_1.default.execute('UPDATE uae_companies SET display_order = ? WHERE id = ?', [displayOrder, companyIdNum]);
...
async function updateSingleOrderField(companyId, orderValue, field) {
    const [currentRows] = await database_1.default.execute(`SELECT COALESCE(${field}, 0) AS current_value FROM uae_companies WHERE id = ? LIMIT 1`, [companyId]);
    const current = currentRows[0];
    if (!current) {
    ...
    if (orderValue !== Number(current.current_value || 0) && orderValue > 0) {
        ...
        AS total`);
            ...
            if (Number(current.current_value || 0) > 0)
                total -= 1;
            if (total >= 9) {
        ...
        const [conflicts] = await database_1.default.execute(`SELECT 'company_profiles' AS source, id, company_name AS name
       FROM company_profiles
       ...
       LIMIT 1`, [orderValue, orderValue, companyId]);
        if (conflicts.length > 0) {
            const conflict = conflicts[0];
            throw new Error(`${field}#CONFLICT#${orderValue}#${conflict.name}`);
        }
    }
    await database_1.default.execute(`UPDATE uae_companies SET ${field} = ? WHERE id = ?`, [orderValue, companyId]);
}
async function updateCompanyHomeDisplayOrder(req, res) {
    try {
...
async function updateCompanyListDisplayOrder(req, res) {
    try {
        const companyId = Number(req.params.companyId);
...
async function getHomeOrderCount(_req, res) {
    try {
...
async function listCompanyApplications(req, res) {
    try {
        ...
        const [countRows] = await database_1.default.execute(`SELECT COUNT(*) as total FROM company_applications ca ${where}`, params);
        const total = countRows[0].total;
        const [rows] = await database_1.default.execute(`SELECT ca.*, u.full_name as user_name, u.email as user_email
...
async function reviewCompanyApplication(req, res) {
    try {
        const { id } = req.params;
        ...
        await database_1.default.execute('UPDATE company_applications SET status = ?, admin_notes = ?, reviewed_by = ?, reviewed_at = NOW() WHERE id = ?', [status, admin_notes || null, adminId, id]);
...
async function bindUserToCompany(req, res) {
    try {
        const { companyId } = req.params;
        ...
        // Check user exists
        const [userRows] = await database_1.default.execute('SELECT id, role FROM users WHERE id = ?', [userId]);
        if (userRows.length === 0) {
            return res.status(404).json({ error: 'User not found.' });
        }
        // Check company exists
        const [companyRows] = await database_1.default.execute('SELECT id FROM uae_companies WHERE id = ?', [companyId]);
        if (companyRows.length === 0) {
            return res.status(404).json({ error: 'Company not found.' });
        }
        // Bind
        await database_1.default.execute('UPDATE uae_companies SET owner_user_id = ? WHERE id = ?', [userId, companyId]);
        await database_1.default.execute("UPDATE users SET role = 'company' WHERE id = ?", [userId]);
        // If there's a pending application from this user, link it
        await database_1.default.execute("UPDATE company_applications SET linked_company_id = ?, status = 'approved', reviewed_at = NOW() WHERE user_id = ? AND status = 'pending'", [companyId, userId]);
...
async function getScrapedCompany(req, res) {
    try {
        const [rows] = await database_1.default.execute('SELECT * FROM uae_companies WHERE id = ?', [req.params.companyId]);
        if (rows.length === 0)
            return res.status(404).json({ error: 'Company not found.' });
...
async function editScrapedCompany(req, res) {
    try {
        const { companyId } = req.params;
        ...
        if (year_established !== undefined) {
            ...
            values.push(year_established || null);
        }
        ...
        await database_1.default.execute(`UPDATE uae_companies SET ${updates.join(', ')} WHERE id = ?`, values);
        const [rows] = await database_1.default.execute('SELECT * FROM uae_companies WHERE id = ?', [companyId]);
        res.json({ company: rows[0] });
    }
    catch (error) {
...
async function getCompanyProfile(req, res) {
    try {
        const [rows] = await database_1.default.execute(`SELECT cp.*, u.email as user_email, u.full_name as user_name
       FROM company_profiles cp JOIN users u ON cp.user_id = u.id WHERE cp.id = ?`, [req.params.id]);
        if (rows.length === 0)
...
async function setCompanyCoverImage(req, res) {
    try {
        const { id } = req.params;
        const { url } = req.body || {};
        const value = (typeof url === 'string' && url.trim()) ? url.trim() : null;
        await database_1.default.execute('UPDATE company_profiles SET cover_image_url = ? WHERE id = ?', [value, id]);
...
async function setDirectoryCompanyCoverImage(req, res) {
    try {
        const { companyId } = req.params;
        const { url } = req.body || {};
        const value = (typeof url === 'string' && url.trim()) ? url.trim() : null;
        await database_1.default.execute('UPDATE uae_companies SET cover_image = ? WHERE id = ?', [value, companyId]);
...
async function editCompanyProfile(req, res) {
    try {
        ...
        if (company_type !== undefined) {
            ...
            values.push(JSON.stringify(_typeArr));
            updates.push('company_types = ?');
            values.push(JSON.stringify(_typeArr));
        }
        if (trade_license_number !== undefined) {
            updates.push('trade_license_number = ?');
            values.push(trade_license_number || null);
        }
        if (establishment_year !== undefined) {
            updates.push('establishment_year = ?');
            values.push(establishment_year || null);
        }
        if (status !== undefined) {
            updates.push('status = ?');
            values.push(status);
...
// Unbind company
async function unbindCompany(req, res) {
    try {
        const { companyId } = req.params;
        // Get current owner
        const [rows] = await database_1.default.execute('SELECT owner_user_id FROM uae_companies WHERE id = ?', [companyId]);
        const company = rows[0];
        if (!company)
            return res.status(404).json({ error: 'Company not found.' });
        if (company.owner_user_id) {
            // Reset user role to 'user' (unless they have other company bindings)
            const [otherCompanies] = await database_1.default.execute('SELECT id FROM uae_companies WHERE owner_user_id = ? AND id != ?', [company.owner_user_id, companyId]);
            if (otherCompanies.length === 0) {
                await database_1.default.execute("UPDATE users SET role = 'user' WHERE id = ?", [company.owner_user_id]);
            }
        }
        await database_1.default.execute('UPDATE uae_companies SET owner_user_id = NULL WHERE id = ?', [companyId]);
...
async function getCompanyProfileFullDetail(req, res) {
    try {
        const { id } = req.params;
        const [companyRows] = await database_1.default.execute(`SELECT cp.*, u.email as user_email, u.full_name as user_name
       ...
       WHERE cp.id = ?`, [id]);
        ...
        const [projectRows] = await database_1.default.execute(`SELECT id, title, description, style, location, year, images, tags, status, rejection_reason, is_published, is_portfolio_hidden, created_at
       FROM projects WHERE company_profile_id = ? AND deleted_at IS NULL ORDER BY created_at DESC`, [id]);
...
async function getCompanyFullDetail(req, res) {
    try {
        const { companyId } = req.params;
        // Get company info
        const [companyRows] = await database_1.default.execute(`SELECT c.*, u.full_name as owner_name, u.email as owner_email, u.id as owner_id
       FROM uae_companies c
       LEFT JOIN users u ON c.owner_user_id = u.id
       WHERE c.id = ?`, [companyId]);
        ...
        let projects = [];
        const [profileRows] = await database_1.default.execute(`SELECT id
       FROM company_profiles
       WHERE linked_uae_company_id = ?
          OR (? IS NOT NULL AND user_id = ?)`, [companyId, company.owner_user_id, company.owner_user_id]);
        const profileIds = profileRows.map((row) => Number(row.id)).filter(Boolean);
        if (profileIds.length > 0) {
            const placeholders = profileIds.map(() => '?').join(',');
            const [projectRows] = await database_1.default.execute(`SELECT id, title, description, style, location, year, images, tags, status, rejection_reason, is_portfolio_hidden, created_at
         FROM projects
         WHERE company_profile_id IN (${placeholders}) AND deleted_at IS NULL
         ORDER BY created_at DESC`, profileIds);
            ...
            const [designerRows] = await database_1.default.execute('SELECT id FROM designers WHERE user_id = ?', [company.owner_user_id]);
            if (designerRows.length > 0) {
                const designerId = designerRows[0].id;
                const [projectRows] = await database_1.default.execute(`SELECT id, title, description, style, location, year, images, tags, status, rejection_reason, is_portfolio_hidden, created_at
           FROM projects WHERE designer_id = ? AND deleted_at IS NULL ORDER BY created_at DESC`, [designerId]);
                projects = projectRows.map((p) => ({
                    ...p,
                    images: sanitizeProjectImages(p.images),
                    tags: toStringArray(p.tags),
        ...
        if (projects.length === 0) {
            const { portfolio_categories } = (0, publicCompaniesSerialization_1.extractPortfolioData)(company.portfolio_images);
            const portfolioItems = Object.entries(portfolio_categories).flatMap(([category, items]) => (items || []).map((item) => ({
...
async function deleteDirectoryPortfolioImage(req, res) {
    try {
        const { companyId } = req.params;
        const { url } = req.body;
        ...
        const [rows] = await database_1.default.execute('SELECT portfolio_images FROM uae_companies WHERE id = ?', [companyId]);
        const row = rows[0];
        if (!row)
            return res.status(404).json({ error: 'Company not found.' });
        ...
        const normalizeStored = (raw) => normalizeLegacyImageUrl((0, publicCompaniesSerialization_1.sanitizeCompanyImage)(String(raw ?? '')));
        if (Array.isArray(parsed)) {
            // Legacy flat array of URL strings
            parsed = parsed.filter((u) => normalizeStored(u) !== url);
        }
            ...
            for (const key of Object.keys(parsed)) {
                ...
                if (Array.isArray(entry)) {
                    parsed[key] = entry.filter((item) => normalizeStored(item?.url) !== url);
                }
                else if (entry && typeof entry === 'object' && Array.isArray(entry.items)) {
                    entry.items = entry.items.filter((item) => normalizeStored(item?.url) !== url);
                }
            }
        }
        await database_1.default.execute('UPDATE uae_companies SET portfolio_images = ? WHERE id = ?', [JSON.stringify(parsed), companyId]);
        res.json({ ok: true });
    }
    catch (error) {
...
async function resolveDesignerIdForProfile(companyProfileId) {
    const [rows] = await database_1.default.execute('SELECT user_id FROM company_profiles WHERE id = ? LIMIT 1', [companyProfileId]);
...
async function getAdminProject(req, res) {
    try {
        ...
        const [rows] = await database_1.default.execute('SELECT * FROM projects WHERE id = ? AND company_profile_id = ? AND deleted_at IS NULL', [projectId, companyId]);
        if (rows.length === 0) {
...
async function updateAdminProject(req, res) {
    try {
        ...
        const [existing] = await database_1.default.execute('SELECT id FROM projects WHERE id = ? AND company_profile_id = ? AND deleted_at IS NULL', [projectId, companyId]);
        ...
        if (Array.isArray(images)) {
            ...
            const persistedUrls = await (0, projectImageStorage_1.persistProjectImages)(images, {
                designerId,
                projectId: Number(projectId),
            });
            // Preserve AI tag metadata for existing images
            const [currentRows] = await database_1.default.execute('SELECT images FROM projects WHERE id = ? AND company_profile_id = ?', [projectId, companyId]);
            const currentRaw = currentRows[0]?.images;
            ...
            finalImages = persistedUrls.map((img) => {
...
async function createAdminProject(req, res) {
    try {
        ...
        const [companyRows] = await database_1.default.execute('SELECT id, status FROM company_profiles WHERE id = ?', [companyId]);
        if (companyRows.length === 0) {
        ...
        if (Array.isArray(images)) {
            const designerId = await resolveDesignerIdForProfile(Number(companyId));
            persistedImages = await (0, projectImageStorage_1.persistProjectImages)(images, { designerId });
        }
        const [result] = await database_1.default.execute(`INSERT INTO projects (company_profile_id, title, description, style, location, year, images, tags, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [
            companyId,
            title ?? null,
            description ?? null,
        ...
        if (title) {
            ...
            await database_1.default.execute('UPDATE projects SET slug = ? WHERE id = ?', [slug, newProjectId]);
        }
...
async function deleteAdminProject(req, res) {
    try {
        ...
        await database_1.default.execute('UPDATE projects SET deleted_at = NOW() WHERE id = ? AND company_profile_id = ?', [projectId, companyId]);
        // Write audit log
        const adminId = req.admin?.id || 0;
        const adminName = req.admin?.username || req.admin?.full_name || 'unknown';
        await database_1.default.execute(`INSERT INTO admin_audit_log (admin_id, admin_name, action, target_type, target_ids, metadata)
...
async function restoreAdminProject(req, res) {
    try {
        const { companyId, projectId } = req.params;
        const [existing] = await database_1.default.execute('SELECT id FROM projects WHERE id = ? AND company_profile_id = ?', [projectId, companyId]);
        if (existing.length === 0) {
            return res.status(404).json({ error: 'Project not found.' });
        }
        await database_1.default.execute('UPDATE projects SET deleted_at = NULL WHERE id = ? AND company_profile_id = ?', [projectId, companyId]);
        res.json({ ok: true });
    }
    catch (error) {
...
async function toggleCompanyProfileSigned(req, res) {
    try {
        const { id } = req.params;
        const { is_signed } = req.body;
        await database_1.default.execute('UPDATE company_profiles SET is_signed = ? WHERE id = ?', [is_signed ? 1 : 0, id]);
        res.json({ ok: true });
    }
    catch (error) {
...
async function toggleCompanyProfilePublished(req, res) {
    try {
        const { id } = req.params;
        const { is_published } = req.body;
        await database_1.default.execute('UPDATE company_profiles SET is_published = ? WHERE id = ?', [is_published ? 1 : 0, id]);
        res.json({ ok: true });
        // Fire-and-forget IndexNow notification — isolated from main error handler
        (async () => {
            try {
                if (is_published) {
                    const [rows] = await database_1.default.execute('SELECT slug FROM company_profiles WHERE id = ?', [id]);
                    const slug = rows[0]?.slug;
                    if (slug)
                        await (0, indexNow_1.notifyIndexNow)([`/companies/${slug}`]);
                }
            }
            catch (err) {
...
async function toggleDirectoryPublished(req, res) {
    try {
        const { companyId } = req.params;
        const { is_published } = req.body;
        await database_1.default.execute('UPDATE uae_companies SET is_published = ? WHERE id = ?', [is_published ? 1 : 0, companyId]);
        res.json({ ok: true });
        // Fire-and-forget IndexNow notification — isolated from main error handler
        (async () => {
            try {
                if (is_published) {
                    const [rows] = await database_1.default.execute('SELECT slug FROM uae_companies WHERE id = ?', [companyId]);
                    const slug = rows[0]?.slug;
                    if (slug)
                        await (0, indexNow_1.notifyIndexNow)([`/companies/${slug}`]);
                }
...
async function toggleProjectPublished(req, res) {
    try {
        const { projectId } = req.params;
        const { is_published } = req.body;
        await database_1.default.execute('UPDATE projects SET is_published = ? WHERE id = ?', [is_published ? 1 : 0, projectId]);
        res.json({ ok: true });
        // Fire-and-forget IndexNow notification — isolated from main error handler
        (async () => {
            try {
                if (is_published) {
                    const [rows] = await database_1.default.execute(`SELECT p.slug AS project_slug, cp.slug AS company_slug
             FROM projects p
             LEFT JOIN company_profiles cp ON p.company_profile_id = cp.id
             WHERE p.id = ?`, [projectId]);
                    const row = rows[0];
                    if (row?.project_slug && row?.company_slug) {
                        await (0, indexNow_1.notifyIndexNow)([`/companies/${row.company_slug}/${row.project_slug}`]);
                    }
                }
            }
            catch (err) {
...
async function getWeightConfigList(req, res) {
    try {
        const [rows] = await database_1.default.execute('SELECT * FROM weight_config ORDER BY id');
...
async function updateWeightConfig(req, res) {
    try {
        const { key } = req.params;
        const { value } = req.body;
        await database_1.default.execute('UPDATE weight_config SET config_value = ? WHERE config_key = ?', [Number(value), key]);
        res.json({ ok: true });
    }
    catch (error) {
...
async function triggerWeightRecalculation(req, res) {
    try {
        const result = await (0, weightCalculator_1.calculateAllWeights)();
        res.json(result);
    }
    catch (error) {
 ...
 */
async function adminCrmProvisionCompany(req, res) {
    try {
        const id = parseInt(req.params.id);
        if (!id)
            return res.status(400).json({ error: 'Invalid company id' });
        const [rows] = await database_1.default.execute('SELECT id, crm_tenant_id, status FROM company_profiles WHERE id = ? AND deleted_at IS NULL', [id]);
        const company = rows[0];
        if (!company)
            return res.status(404).json({ error: 'Company not found' });
        if (company.status !== 'approved')
            return res.status(400).json({ error: 'Company must be approved before CRM provisioning' });
        const { provision } = await Promise.resolve().then(() => __importStar(require('../lib/crmIntegrationService')));
