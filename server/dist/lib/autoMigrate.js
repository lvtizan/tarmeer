...
exports.runAutoMigrate = runAutoMigrate;
...
const REQUIRED_COLUMNS = [
    ...
    // Generic pending-actions queue (carry-over scaffold, replaces pending_profile)
    { table: 'users', column: 'pending_actions', type: 'JSON NULL' },
    ...
    { table: 'supplier_products', column: 'video_url', type: 'VARCHAR(500) NULL' },
...
async function columnExists(table, column) {
    const [rows] = await database_1.default.execute('SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?', [table, column]);
    return rows.length > 0;
}
async function indexExists(table, indexName) {
    const [rows] = await database_1.default.execute('SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND INDEX_NAME = ?', [table, indexName]);
    return rows.length > 0;
}
async function isColumnNullable(table, column) {
    const [rows] = await database_1.default.execute('SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?', [table, column]);
    const row = rows[0];
...
async function runAutoMigrate(options = {}) {
    ...
    try {
        ...
        try {
            await database_1.default.execute(`INSERT IGNORE INTO weight_config (config_key, config_value, description) VALUES
        ...
        try {
            await database_1.default.execute(`INSERT IGNORE INTO company_types (slug, label, sort_order) VALUES
        ...
        try {
            await database_1.default.execute(`INSERT IGNORE INTO company_services (name, sort_order) VALUES
          ('Interior Design',          1),
          ('Architecture',             2),
          ('Fit-Out',                  3),
          ('Renovation',               4),
        ...
        try {
            ...
            for (const [cat, names] of categoryMap) {
                ...
                await database_1.default.execute(`UPDATE company_services SET category = ? WHERE name IN (${placeholders}) AND (category IS NULL OR category = '')`, [cat, ...names]);
            }
        }
        catch { /* ignore */ }
        ...
        try {
            await database_1.default.execute(`UPDATE company_service_categories SET name = 'Build Only'    WHERE name = 'Construction'`);
            await database_1.default.execute(`UPDATE company_services SET category = 'Build Only'          WHERE category = 'Construction'`);
            await database_1.default.execute(`UPDATE company_service_categories SET name = 'Design Only'   WHERE name = 'Design & Planning'`);
            await database_1.default.execute(`UPDATE company_services SET category = 'Design Only'         WHERE category = 'Design & Planning'`);
            await database_1.default.execute(`UPDATE company_service_categories SET name = 'Extension'     WHERE name = 'Extensions'`);
            await database_1.default.execute(`UPDATE company_services SET category = 'Extension'           WHERE category = 'Extensions'`);
        }
        catch { /* ignore */ }
        // 7c. Seed company_service_categories from known category list (idempotent)
        try {
            await database_1.default.execute(`INSERT INTO company_service_categories (name, sort_order, is_enabled) VALUES
          ('Design Only',          1, 1),
          ('Build Only',           2, 1),
          ('Design & Build',       3, 1),
          ('Renovation',           4, 1),
        ...
        // 7c-2. Add group_value to supplier_categories
        {
            ...
            if (!gcExists) {
                try {
                    await database_1.default.execute(`ALTER TABLE supplier_categories ADD COLUMN group_value VARCHAR(50) NULL`);
                    changes++;
                    console.log(`${TAG} Added group_value to supplier_categories`);
                }
                catch (e) {
                    console.error(`${TAG} Failed to add group_value:`, e);
                }
            }
        }
      ...
      `);
        ...
        // 8. Add is_portfolio_hidden to projects
        {
            ...
            if (!phExists) {
                try {
                    await database_1.default.execute(`ALTER TABLE projects ADD COLUMN is_portfolio_hidden TINYINT NOT NULL DEFAULT 0`);
                    changes++;
                    console.log(`${TAG} Added is_portfolio_hidden to projects`);
                }
                catch (e) {
                    console.error(`${TAG} Failed to add is_portfolio_hidden:`, e);
                }
            }
        }
