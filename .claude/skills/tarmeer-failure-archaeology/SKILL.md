...
- **根因**：`7a8cc7ed8` 为避免多上传区重复处理而把原有全局图片粘贴改为必须先激活上传区，破坏了原有图片粘贴；同时目录页沿用仅提取图片的剪贴板分支，且没有把目录大小上限传入前端校验。
- **修复**：恢复默认图片上传区的全局图片粘贴语义；仅目录页显式启用 PDF 剪贴板接收与 60MB 前后端上限，超限文件直接显示明确限制提示。
