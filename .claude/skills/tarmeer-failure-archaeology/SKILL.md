...
### FA-26 营业执照旧 Base64 上传绕过公共压缩（2026-09-08）
- **现象**：产品图片已支持超大图自动压缩，但供应商“资料 → 营业执照”仍可能在约 15MB 原文件时因 Base64 膨胀撞上 20MB JSON body 限制；错误处理也不统一。
- **根因**：该单文件入口使用独立的 `FileUploadButton`，把文件读为 `data_url` JSON，而非复用公共 Multipart 图片上传策略。
- **修复**：控件改为 Multipart 上传并在图片时复用 `prepareImageForUpload`；后端 `upload-license` 改接 `multer` 文件、使用 MIME 白名单与 60MB 校验，同时短期兼容已打开旧页面的 Base64 请求。
- **预防**：声称“全站统一上传”前，必须 `rg` 检查所有上传控件和 `data_url` 写入口；同一用户流程的上传不得混用 Base64 JSON 与 Multipart，除非有清晰的大小和错误处理契约。

### FA-27 供应商异步操作假成功或无反馈（2026-09-08）
- **现象**：翻译服务异常时英文输入框空白且无提示；产品、项目、目录删除失败时前端先隐藏条目，刷新后又出现。
- **根因**：翻译直接 `res.json()` 且缺少错误分支；删除请求未检查 `response.ok`，先乐观修改了本地状态。
- **修复**：翻译按响应状态和 JSON 解析失败给出可见提示并允许手填；三类删除均在服务端成功后才修改本地列表，失败显示页面级错误。
- **预防**：供应商门户的每个异步写操作都必须同时验证 HTTP 状态、非 JSON 错误体和状态提交顺序；不能用“请求发出”替代“服务端成功”。

### FA-28 前端构建未完成即暴露不完整 `.next`，全站短暂 500（2026-09-08）
- **现象**：11:00–11:03 巡检同时报出公司目录、作品集、博客、登录等 18 个前台入口 HTTP 500；错误日志显示缺少 `.next/required-server-files.json`、`BUILD_ID` 和 `/500.html`。
- **根因**：前台构建过程被中断后留下了半成品 `.next` 目录，运行中的 Next 服务读取到缺失的构建文件；当时尚未确认构建成功和 BUILD_ID 已生成。
- **修复**：在服务器独立后台完成完整构建，等待构建退出码为 0 且生成新的 BUILD_ID 后才重启 `tarmeer-next`；恢复后逐页验证告警中的 18 个入口均为 HTTP 200。
- **预防**：前端发布必须用不受 SSH 交互会话影响的构建任务，并以“构建 exit=0 + BUILD_ID 存在且更新”为唯一重启门禁；失败或仍在构建中时绝不重启或暴露构建目录。发布后必须对巡检关键路由做状态码回归。

### FA-29 材料首页把“热门权重”误用为“全量目录”排序（2026-09-08）
- **现象**：后台已有不少新上传商品，但材料首页长期只显得像少数旧供应商；新商品被高权重旧供应商遮蔽。
- **根因**：公开产品 feed 同时供热门推荐和全量目录使用，SQL 固定按 `supplier_profiles.weight_score` 优先排序，首页没有独立的全量目录排序语义。
- **修复**：移除公开 feed 的权重排序，统一以 `supplier_products.id DESC`（最新上架）返回；首页默认全量分页，左侧分类直接按同一规则筛选。
- **预防**：展示“全部”内容的接口不得复用策展/权重排序；排序目的必须在接口契约中明确，并用新建内容在首屏的浏览器验收覆盖。

### FA-30 材料分类二级面板固定在目录顶端，横移时无法点击（2026-09-08）
- **现象**：用户悬停较靠下的材料分类时，二级面板仍浮在左栏顶部；鼠标从当前分类横移到面板容易离开命中区，面板立即消失，二级链接无法稳定点击。
- **根因**：面板绝对定位在整个目录根节点上，固定使用 `top-0`，而非以当前分类行作为定位锚点。
- **修复**：将桌面二级面板移动为活动分类 `<li className="relative">` 的子节点，继续保留其 `pl-4` 命中桥；面板随当前行下移，横移全程仍在同一外层 hover 容器内。
- **预防**：带 hover 的侧栏二级面板必须以触发项为定位祖先，且触发项到面板的视觉间隔必须属于同一可命中 DOM 区域；验收要覆盖靠下菜单项的“行 → 间隔 → 面板”鼠标路径。

### FA-31 商品来源进入供应商详情后，返回被写死为供应商列表（2026-09-08）
- **现象**：用户在 Products tab 点击商品卡的供应商，详情页点击 Back 却回到 Suppliers tab，丢失原浏览上下文。
- **根因**：商品卡链接未携带来源；供应商详情的返回和面包屑硬编码为 `/materials?tab=suppliers`。
- **修复**：产品来源的供应商链接统一使用 `supplierFromProductsHref()` 追加受控的 `?from=products`；详情页只识别该白名单值，返回和面包屑据此回 Products，其它来源仍默认 Suppliers。
- **预防**：页面存在多个入口时，任何详情页返回动作都必须由受控来源参数或浏览器历史决定，禁止写死某个列表；测试需覆盖“列表 → 产品详情 → 供应商详情 → 返回”的完整链路。

### FA-32 分类切换先清空商品，导致 Footer 跳出与页面闪白（2026-09-08）
- **现象**：材料页切换分类时右侧先变空、Footer 突然上移，用户误以为整页刷新或页面跳动。
- **根因**：分类请求启动就清空 `products`，加载区高度塌陷；若简单保留旧卡片，又会引入跨国家泄漏、失败后标题与卡片不一致及分页混类风险。
- **修复**：以 `displayedCountry + displayedCategory` 作为已提交内容身份；仅同国家切换时保留旧卡片并显示更新标记，新数据成功后原子替换标题/数据/页码；跨国家立即隐藏旧内容；刷新期和身份不一致时禁用分页，失败提供 Retry。
- **预防**：异步列表的“无感刷新”必须将**显示身份、数据、总数、页码**作为同一提交单元处理；禁止只保留数组而让标题/分页先切换。跨国家数据在请求期间也不得渲染旧内容。

### FA-33 默认商品流被单一供应商垄断（2026-09-08）
- **现象**：默认“全部商品”按全局最新上架排序；某供应商批量上传后，其素材占满首屏，其他供应商几乎没有曝光机会。
- **根因**：发现流将上架时间作为唯一排序维度，没有供应商轮换规则。
- **修复**：默认 Hub 显式请求 `balanced=1`；后端在保留国家、状态和图片过滤后，按供应商对产品新旧排名，每两件组成一轮排序。具体品类、场景和搜索均不启用该参数，继续返回完整、按新旧排序的结果。
- **预防**：任何发现流公平性策略必须是显式 opt-in；不得隐式改变搜索、品类浏览、场景筛选或国家隔离的查询语义。分页必须使用同一确定性排序，避免重复或跳项。

### FA-34 供应商目录只依赖浏览器 PDF 解析，部分有效 PDF 在前端黑屏（2026-09-14）
- **现象**：FangyuanShiguang@tarmeer.com 上传的 27 页目录已入库且原文件可下载，供应商详情仍显示 “Unable to load this catalog”。
- **根因**：上传链路只保存 PDF 原件，未生成阅读器所需的页图和 manifest；前端回退到各浏览器的 pdf.js 解析，失败被吞成通用黑屏错误。
- **修复**：目录创建、管理员代传/替换和历史目录读取均触发服务端 `pdftoppm` 页图转换；页面与 manifest 原子发布，前端轮询转换结果、读取稳定页图，并提供原 PDF 下载兜底。
- **预防**：任何面向访客的供应商目录必须由服务端生成可直接展示的资源，不能把兼容性关键路径交给浏览器 PDF 解析；新增目录路径要覆盖供应商上传、管理员代传、替换及历史回补四种入口。

### FA-35 目录删除可在数据库删除后遗留公开页图（2026-09-14）
- **现象**：若进程在目录记录删除后、静态页图清理前中断，旧 `pages/<catalogId>` 可继续被已知 URL 访问，且后续数据库驱动的修复任务无法再发现它。
- **根因**：删除路径先删除数据库行，再删除公开静态目录；公开文件的可访问性不受数据库约束。
- **修复**：删除统一改为先原子移动公开目录到私有隔离区，再删除记录和私有原件；失败时恢复隔离目录，成功后清理。渲染 worker 额外有界扫描并清理没有对应目录记录的公开根目录。
- **预防**：所有公开静态资源的撤回/删除必须先断开公开路径再修改或删除数据库记录；对于历史残留必须有独立的无数据库行扫描兜底。

### FA-36 管理后台供应商传图链路与个人中心分叉（2026-09-16）
- **现象**：供应商个人中心可拖拽、粘贴和批量上传图片，管理后台代录时仍使用另一套上传入口，出现大图失败、文件夹拖放不可用、重复粘贴和保存期间误操作等不一致问题。
- **根因**：两个入口各自维护上传交互和服务端校验，后台还允许客户端直接提交图片 URL，缺少文件归属、完整解码和状态锁定。
- **修复**：管理后台复用统一 `ImageUploadZone`，增加唯一粘贴接收者、逐文件容错和上传状态回调；后端统一 20MB 接收、Sharp 完整解码转码、UUID 命名、目标供应商目录归属校验及项目图片数量上限。
- **预防**：新增或修改供应商图片入口时必须先全量搜索个人中心与管理后台的所有入口，统一复用上传组件和后端处理函数；用例需覆盖点击、拖拽、文件夹、粘贴、超大图压缩、部分失败、跨供应商 URL 和上传中关闭/保存。

### FA-37 公开供应商素材残留真实身份与批发字段（2026-09-17）
- **现象**：供应商公开详情改为通用名称后，商品翻译、规格、项目名称和隐藏描述仍可能出现真实供应商名称、批发价、起订量或内部导入标记。
- **根因**：旧脱敏只处理少数顶层原文字段，未递归覆盖翻译对象、规格数组和项目双语字段；页面的 JSON-LD 与无障碍隐藏文本又直接输出了原始描述。
- **修复**：抽取共享递归脱敏函数，统一覆盖公开商品、项目及其嵌套字段；展示层再集中清理价格、MOQ 和内部导入元数据，并停止在服务端结构化数据及隐藏节点输出供应商原始描述。
- **预防**：供应商公开接口新增字段时必须同时纳入递归脱敏测试；验收需检查页面可见文本、SSR HTML、JSON-LD 和 API 原始响应，不能只看卡片视觉结果。

### FA-38 询单入口把买家强制带离当前浏览位置（2026-09-18）
- **现象**：买家点击供应商详情页的“Send Inquiry”后，页面滚动到底部或超宽屏侧栏表单，失去当前商品浏览上下文；入口还需滚动后才出现，关闭后无法恢复。
- **根因**：询单 CTA 被实现成对断点相关重复表单的 `scrollIntoView`，并沿用了滚动阈值和永久 dismiss 状态，没有按上下文动作设计交互。
...
### FA-41 材料目录与供应商详情的页面尺度割裂（2026-09-20）
- **现象**：供应商详情已经使用宽屏商品展示，材料目录仍限制在较窄容器、宽侧栏和小型盒装卡片中；同一浏览链路上下页视觉体量不一致，目录显得局促。
- **根因**：详情页改版时没有同步目录页的容器、侧栏和网格口径，两个页面各自保留独立布局；商品卡还未给空标题、空价格保留稳定元数据槽位。
- **修复**：目录升级为 1920px 宽屏容器、240px 侧栏和 2–5 列自适应网格，改用更大的 4:3 无外框商品卡；默认流与搜索流复用同一商品卡，并统一标题兜底与元数据行高。
- **预防**：同一业务域的列表/详情改版必须联合验收视觉尺度；桌面记录容器、侧栏和卡片宽度，移动端检查横向溢出，空字段状态纳入布局用例。具体规则写入 `tarmeer-ui-conventions`。

### FA-42 材料目录首屏按钮截断商品发现流（2026-09-20）
- **现象**：材料目录虽然有数百个商品，但首批 24 个之后必须点击 “Load more products”；用户容易把按钮上方当成完整目录，认为平台只有少量商品。
- **根因**：分页能力只实现成显式按钮，没有把长目录按连续浏览场景设计；加载失败和重复请求也未与自动翻页分别建模。
- **修复**：以提前 900px 的 `IntersectionObserver` 触发下一页，移除手动加载按钮；增加请求并发锁、跨页 ID 去重、加载状态与独立失败重试，分类/国家切换时重置观察状态。
- **预防**：发现流默认采用连续自动加载；实现时必须覆盖自动触发、无重复、请求失败可恢复、筛选切换不串页以及无障碍状态提示，规则写入 `tarmeer-ui-conventions`。

### FA-43 材料目录分类导航随商品流被推离视野（2026-09-22）
- **现象**：用户向下浏览材料目录时，左侧分类导航随页面一起离开，不能随时切换品类。
- **根因**：目录作为 grid item 使用默认 stretch 对齐，缺少 sticky 定位与顶部偏移，无法作为长内容流的常驻导航。
...
### FA-45 供应商目录上传对剪贴板 PDF 与超限文件给出误导性失败（2026-09-24）
- **现象**：供应商目录页无法直接粘贴 PDF；超过分片协议上限的文件上传后只显示 `Invalid catalog upload chunk`，用户无法判断是文件大小问题。
- **根因**：`7a8cc7ed8` 为避免多上传区重复处理而把原有全局图片粘贴改为必须先激活上传区，破坏了原有图片粘贴；同时目录页沿用仅提取图片的剪贴板分支，且没有把 60MB 后端上限传入前端校验。
- **修复**：恢复默认图片上传区的全局图片粘贴语义；仅目录页显式启用 PDF 剪贴板接收与 60MB 前端上限，超限文件直接显示明确限制提示。
- **预防**：共享组件为解决多实例问题时，必须建立既有无焦点粘贴用例；复用上传区接入非图片文件时，必须显式声明剪贴板接收范围和接口大小上限，禁止把协议校验错误直接暴露为用户可见的唯一错误。
