...
### FA-18 admin 采购线索 500——跨表 country 列 collation 不一致（2026-07-25）
- **现象**：Admin「采购线索」页红字 `Unable to connect to the server`（其它 admin 列表正常）；后端 `GET /api/admin/sourcing-requests` 认证正常(401无token)但带 token 时 500。
- **根因**：`adminListSourcingRequests` 的 JOIN 有跨表字符串比较 `psp.country = sr.country` / `sp.country = sr.country`；生产库 `sourcing_requests.country`=`utf8mb4_unicode_ci`、`supplier_profiles.country`=`utf8mb4_0900_ai_ci` → `ER_CANT_AGGREGATE_2COLLATIONS: Illegal mix of collations` → 500。前端 catch 成笼统"Unable to connect"。**正是 AGENTS.md「跨表字符串比较列 vs 列」预警坑的实例。**
- **修复**：JOIN 的两处比较加 `COLLATE utf8mb4_unicode_ci` 统一（`sp.country COLLATE utf8mb4_unicode_ci = sr.country`）；rsync `sourcingRequestController.js` + `pm2 restart tarmeer-api`；生产实跑查询 OK（0 行=表本空，无人提交）。
- **预防**：① **新写/改任何跨表 `X.country = Y.country`（或任意列 vs 列字符串比较）前，先在生产查 `INFORMATION_SCHEMA.COLUMNS` 比对 `COLLATION_NAME`**，不一致就地加 `COLLATE`；② 建表/加列时统一 collation（历史遗留 unicode_ci vs 0900_ai_ci 分叉）；③ 前端"Unable to connect"多为后端 500 而非真断网——先 curl 端点看是 401 还是 500 再判方向；④ fieldAdminController 也有 `uc.country=ci.country` 同类比较，需核对其 collation（未报故障=可能一致，但列入排查清单）。
