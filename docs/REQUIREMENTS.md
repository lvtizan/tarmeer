# Tarmeer 4.0 — 全站需求说明（AI 可复用）

本文档描述 Tarmeer 4.0 网站的全部功能、结构、设计与交互，供其他 AI 或开发者复现或扩展项目时使用。
...
- **动效**：Framer Motion（可选）
- **图标**：Lucide React
- **数据**：当前为前端 Mock（设计器后台用 React Context）；规划中为 MySQL + 阿里云 OSS 存图 URL
...
| 用途 | 值 | 说明 |
|------|-----|------|
| 主色（按钮、CTA） | `#b8864a` | CSS var `--color-tarmeer-primary` / `--color-tarmeer-primary-btn` |
...
- 全站图片要求「高端大气」；所有关键图片需 `onError` 回退（占位图或隐藏），避免裂图

---

## 4. 全局常量（Contact & Links）
...
### 5.2 设计师后台（独立 Layout，无站点头部/Footer）
...
### 14.2 作品集列表（/designer/projects）

- 所有项目以列表或卡片展示；每项有编辑、删除；「Add Project」链到 `/designer/upload`。

### 14.3 个人资料（/designer/profile）

- 编辑：姓名、职称、邮箱、电话、城市、地址、简介；头像上传（本地预览，如 FileReader）。

### 14.4 上传/编辑项目（/designer/upload 与 /designer/upload/:id）
...
本文档覆盖 Tarmeer 4.0 的页面结构、路由、设计系统、数据模型与关键交互。实现时按章节对照即可保证与其他 AI 或既有实现一致、可复用。
