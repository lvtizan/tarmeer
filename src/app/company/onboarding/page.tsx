...
export default function CompanyOnboardingPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const meData = meRes as { user?: { full_name?: string; phone?: string } } | null;
        const user = meData?.user || meData as { full_name?: string; phone?: string } | null;
        ...
        const profileData = profileRes as { profile?: Record<string, unknown>; projectCount?: number } | null;
        ...
        if (profile) {
          setProfileData({
            ...
            contact_person: (profile.contact_person as string) || '',
            phone: (profile.phone as string) || '',
          });
          if (profile.company_name) setCompanyName(profile.company_name as string);
          if (profile.contact_person) setContactPerson(profile.contact_person as string);
          if (profile.company_type) setCompanyType(profile.company_type as string);
        }

        if (pending && (!profile?.company_name || !profile?.contact_person)) {
          if (pending.company_name) setCompanyName(pending.company_name as string);
          if (pending.contact_person) setContactPerson(pending.contact_person as string);
          if (pending.company_type) setCompanyType(pending.company_type as string);
          if (pending.phone) {
            const match = GCC_PHONE_OPTIONS.find(o => (pending!.phone as string).startsWith(o.code));
            if (match) { setPhoneRegion(match); setPhoneDigits((pending.phone as string).slice(match.code.length)); }
          }
          try {
  ...
  const addFiles = async (fileList: FileList | File[]) => {
    ...
    if (totalBytes > MAX_TOTAL_UPLOAD_BYTES) { setError(buildUploadSizeMessage(totalBytes)); return; }
    ...
    try {
      ...
      setError((err as { message?: string })?.message || 'Failed to process images.');
    } finally {
      setPrepping(false);
    }
  };
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (result.files.length > 0) {
      if (result.folderName && !projectTitle.trim()) setProjectTitle(result.folderName);
  ...
  const handleStep1 = async () => {
    ...
    try {
      ...
      setProfileData({ company_name: companyName.trim(), contact_person: contactPerson.trim(), phone: fullPhone });
      setStep(2);
    } catch (err: unknown) {
  ...
  const handleStep2 = async () => {
    ...
    try {
      await api.post('/projects', {
        title: projectTitle.trim(), description: '', location: city,
        style: style || null, images, tags: [], status: 'pending',
      });
      await api.post('/auth/company/profile', {
        company_name: profileData.company_name, contact_person: profileData.contact_person,
        phone: profileData.phone, company_type: companyType || 'renovation_company', onboarding_step: 2,
      });
      setStep(3);
    } catch (err: unknown) {
      setError((err as { message?: string })?.message || 'Something went wrong');
  ...
  const handleStep3 = async (skip: boolean) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        company_name: profileData.company_name, contact_person: profileData.contact_person,
        phone: profileData.phone, company_type: companyType || 'renovation_company', onboarding_step: 3,
      };
      if (!skip) {
    ...
    <div className="flex items-center justify-center gap-0 py-6">
      {[1, 2, 3].map((s, i) => (
        <div key={s} className="flex items-center">
          <div className={`w-3 h-3 rounded-full transition-colors ${s <= step ? 'bg-[#b8864a]' : 'bg-stone-300'}`} />
          {i < 2 && <div className={`w-12 h-0.5 transition-colors ${s < step ? 'bg-[#b8864a]' : 'bg-stone-300'}`} />}
        </div>
      ))}
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#faf9f7]">
        <header className="bg-white border-b border-stone-100 h-14 flex items-center px-6">
      ...
      <div className="mx-auto max-w-[640px] px-4 pb-16">
          ...
          <div>
            ...
            <div className="mt-8 space-y-5">
              <div>
                ...
                <input type="text" value={companyName} onChange={e => setCompanyName(e.target.value)} placeholder="Your company name" className={INPUT_CLS} />
              </div>
              <div>
                <label className={LABEL_CLS}>Contact Person *</label>
                <input type="text" value={contactPerson} onChange={e => setContactPerson(e.target.value)} placeholder="Full name" className={INPUT_CLS} />
              </div>
              <div>
                ...
                <div className="grid grid-cols-[130px_1fr] gap-2">
                  ...
                  <input
                    type="tel" inputMode="numeric" value={phoneDigits}
                    onChange={e => { const digits = e.target.value.replace(/\D/g, '').slice(0, phoneRegion.maxDigits); setPhoneDigits(digits); }}
                    maxLength={phoneRegion.maxDigits} placeholder={`Enter ${phoneRegion.maxDigits}-digit number`} className={INPUT_CLS}
                  />
                </div>
              </div>
            </div>

            <button type="button" onClick={handleStep1} disabled={saving} className="btn-primary mt-8 w-full h-[50px] rounded-2xl text-[15px] font-semibold">
        ...
        {step === 2 && (
          <div>
            ...
            <div className="mt-4 rounded-2xl bg-[#b8864a]/[0.08] p-4 mb-6">
              <p className="text-sm text-[#2c2c2c]">The more project photos you upload, the higher you rank in search results.</p>
            </div>

            <div className="space-y-5">
              <div>
                <label className={LABEL_CLS}>Project Title *</label>
                <input type="text" value={projectTitle} onChange={e => setProjectTitle(e.target.value)} placeholder="e.g. Modern Villa in Dubai Hills" className={INPUT_CLS} />
              </div>
              <div>
                <label className={LABEL_CLS}>City *</label>
                <AdminSelect value={city} onChange={setCity} options={CITY_OPTIONS} className="w-full" />
              </div>
              <div>
                <label className={LABEL_CLS}>Style</label>
                <AdminSelect value={style} onChange={setStyle} options={STYLES} className="w-full" />
              </div>
              ...
              <div>
                ...
                <input ref={folderInputRef} type="file" accept="image/*" multiple
                  {...({webkitdirectory:'',directory:''} as React.InputHTMLAttributes<HTMLInputElement>)} className="hidden"
                  onChange={e => { if(e.target.files?.length){ addFiles(e.target.files); e.target.value=''; } }} />
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition ${dropActive ? 'border-[#b8864a] bg-[#b8864a]/5' : 'border-stone-300 hover:border-[#b8864a]/40'}`}
                >
                ...
                <button type="button" onClick={() => folderInputRef.current?.click()} className="mt-2 text-sm font-medium text-[#b8864a] hover:underline">
                  Upload Folder
                </button>

                {prepping && <p className="mt-2 text-sm text-stone-500">Processing images...</p>}
              </div>
            ...
            <button type="button" onClick={handleStep2} disabled={saving || images.length === 0 || !projectTitle.trim()}
        ...
        {step === 3 && (
          <div>
            ...
            <div className="mt-4 rounded-2xl bg-[#b8864a]/[0.08] p-4 mb-6">
              <p className="text-sm text-[#2c2c2c]">A complete profile gets 3x more inquiries and ranks higher in search.</p>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {SERVICES.map(opt => (
                    <button key={opt} type="button" onClick={() => toggleTag(services, setServices, opt)}
                      className={`px-4 py-2 rounded-2xl text-sm font-medium border transition ${services.includes(opt) ? 'border-[#b8864a] bg-[#b8864a] text-white' : 'border-stone-200 bg-stone-50 text-stone-700 hover:border-[#b8864a]/45'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {SPECIALTIES.map(opt => (
                    <button key={opt} type="button" onClick={() => toggleTag(specialties, setSpecialties, opt)}
                      className={`px-4 py-2 rounded-2xl text-sm font-medium border transition ${specialties.includes(opt) ? 'border-[#b8864a] bg-[#b8864a] text-white' : 'border-stone-200 bg-stone-50 text-stone-700 hover:border-[#b8864a]/45'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <input type="url" value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://yourcompany.com" className={INPUT_CLS} />
              </div>

              <div>
                <label className={LABEL_CLS}>Company Type</label>
                <AdminSelect value={companyType} onChange={setCompanyType} options={[{ value: '', label: 'Select type' }, ...COMPANY_TYPES]} className="w-full" />
              </div>
            </div>

            <div className="mt-8 flex gap-3">
              <button type="button" onClick={() => handleStep3(true)} disabled={saving}
              ...
              <button type="button" onClick={() => handleStep3(false)} disabled={saving} className="btn-primary flex-1 h-[50px] rounded-2xl text-[15px] font-semibold">
