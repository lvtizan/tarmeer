...
export default function CompanyArticlesPage() {
  ...
  const loadArticles = useCallback(async () => {
    try {
      const res = await api.get('/articles/mine') as { articles?: Article[] } | null;
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      const res = await api.post('/articles/generate', {}) as { title?: string; content?: string; excerpt?: string } | null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing === 'new') {
        ...
        await api.put(`/articles/${(editing as Article).id}`, body);
      }
          ...
          <div className="grid w-full items-start gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            ...
            <section className="min-w-0 rounded-[24px] border border-stone-200 bg-white p-6 shadow-[0_20px_60px_rgba(28,18,8,0.05)] space-y-5">
              ...
              <div>
                ...
                />
                <p className="mt-1.5 text-xs text-stone-400">Supports Markdown &mdash; ## headings, **bold**, ![image](url)</p>
              </div>
              <div>
