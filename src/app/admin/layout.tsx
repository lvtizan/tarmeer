...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (supplierScoped && !isStandalone
        ...
        && pathname !== '/admin/supplier-report') {
  ...
  useEffect(() => {
    const notifKey = NOTIFICATION_MAP[pathname] ||
      Object.entries(NOTIFICATION_MAP).find(([p]) => pathname.startsWith(p))?.[1];
    if (notifKey) {
    ...
    const todayNewKey = TODAY_NEW_PAGE_MAP[pathname] ??
      (Object.entries(TODAY_NEW_PAGE_MAP).find(([p]) => pathname.startsWith(p))?.[1] as keyof typeof todayNew | undefined);
    if (todayNewKey) {
  ...
  const showTooltip = (event: React.MouseEvent<HTMLSpanElement>, text: string) => {
