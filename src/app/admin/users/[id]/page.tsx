...
export default function AdminUserDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    adminApi.getUserDetail(Number(id))
      .then(setData)
      .catch((err: unknown) => setError(err instanceof Error ? err.message : t('Failed to load user.', '加载用户失败')))
  ...
  const handleStatusToggle = async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : t('Failed to update status', '更新状态失败'));
    } finally {
  ...
  if (error || !data) {
    return (
      <div className="space-y-4">
        <Link href="/admin/users" className="inline-flex items-center gap-1.5 text-sm text-stone-500 hover:text-stone-800">
        ...
        <div className="fixed inset-0 z-[200] bg-black/40 flex items-center justify-center" onClick={() => !deleteLoading && setDeleteModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-[#6b6b6b]">
              {t(
                `This will permanently delete "${user.full_name || user.email}" and all related data.`,
                `将永久删除「${user.full_name || user.email}」及所有关联数据，不可恢复。`
              )}
            </p>
            <div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-sm">
          ...
          <div>
            ...
            <p className="mt-0.5">
              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${ROLE_BADGE[user.role] || ROLE_BADGE.user}`}>
                {user.role}
              </span>
            </p>
          </div>
          <div>
            ...
            <p className="mt-0.5">
              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_BADGE[user.status] || 'bg-stone-100 text-stone-700'}`}>
                {user.status}
              </span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-stone-800 font-medium mt-0.5">
                <Link href={`/admin/designers/${designer.id}`} className="text-[#b8864a] hover:underline">
                  #{designer.id}
                </Link>
              </p>
            </div>
                ...
                return (
                  <div key={p.id} className="rounded-xl border border-stone-200 overflow-hidden">
                    <div className="aspect-[4/3] bg-stone-100 overflow-hidden">
                      {cover ? (
                        <img src={resolveImageUrl(cover)} alt={p.title} className="w-full h-full object-cover" loading="lazy" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-stone-300 text-sm">{t('No image', '无图片')}</div>
                      )}
                    </div>
                    <div className="p-3">
