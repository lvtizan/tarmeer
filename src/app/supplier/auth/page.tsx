...
function setSupplierToken(t: string) {
  if (typeof window !== 'undefined') localStorage.setItem('supplier_token', t);
}

function SupplierAuthForm() {
  ...
  useEffect(() => {
    if (getSupplierToken()) router.replace('/supplier/dashboard');
  }, [router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        ...
        if (typeof window !== 'undefined') localStorage.setItem('supplier_user', JSON.stringify(data.user));
