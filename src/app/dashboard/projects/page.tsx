...
import {
  FolderOpen, ImagePlus, Trash2, Eye, GripVertical, X, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
function reorder<T>(a: T[], f: number, t: number): T[] {
  if (f === t) return a;
  const n = [...a];
...
export default function HomeownerProjectsPage() {
  ...
  const addFiles = async (files: File[]) => {
    ...
    try {
      ...
      if (eb + p.estimatedPayloadBytes > MAX_ESTIMATED_PAYLOAD_BYTES) {
        setNotice(`Too large. Keep under ${formatFileSize(MAX_ESTIMATED_PAYLOAD_BYTES)}.`); return;
      }
  ...
  const hFS = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.length) { await addFiles(Array.from(e.target.files)); e.target.value = ''; }
  };
  const hDrop = async (e: React.DragEvent) => {
    e.preventDefault(); setDropActive(false);
    const r = await getDroppedImageFiles(e);
    if (r.files.length > 0) await addFiles(r.files);
  };
          ...
          <div className="mb-2.5 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-stone-500">
                {imgs.length > 0
                  ? `${imgs.length} photos · ${formatFileSize(imgs.reduce((s, u) => s + estimateDataUrlBytes(u), 0))}`
                  : 'Upload your renovation photos'}
              </p>
            </div>
            {imgs[cover] && (
              <div className="w-[128px] rounded-xl border border-stone-200 bg-stone-50 p-1.5">
                ...
                <div className="mt-1 aspect-video w-full rounded-lg bg-cover bg-center" style={{ backgroundImage: `url(${imgs[cover]})` }} />
              </div>
            )}
          </div>

          {notice && (
            <div className="mb-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 flex justify-between">
              {notice}
              <button type="button" onClick={() => setNotice('')}><X className="w-3 h-3" /></button>
            </div>
          )}

          <input id="ho-up" type="file" accept="image/*" multiple className="hidden" onChange={hFS} disabled={prepping} />
          <input id="ho-folder" type="file" accept="image/*" multiple {...{ webkitdirectory: '', directory: '' } as React.InputHTMLAttributes<HTMLInputElement>} className="hidden" onChange={hFS} disabled={prepping} />
              ...
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-5">
                {imgs.map((url, i) => (
                  <div key={i} draggable
                    ...
                    onDragOver={e => { e.preventDefault(); if (dragO !== i) setDragO(i); }}
                    onDrop={e => { e.preventDefault(); if (dragI !== null) mvImg(dragI, i); setDragI(null); setDragO(null); }}
                    onDragEnd={() => { setDragI(null); setDragO(null); }}
                    className={`group relative aspect-square overflow-hidden rounded-xl border bg-stone-100 transition ${
                      cover === i ? 'border-[#b8864a] ring-2 ring-[#b8864a]/35' : dragO === i ? 'border-[#b8864a]/70' : 'border-stone-200'
                    } ${dragI === i ? 'opacity-80 cursor-grabbing' : 'cursor-grab'}`}>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-4xl rounded-2xl bg-white p-4 shadow-2xl">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm font-semibold text-stone-700">{prevI + 1}/{imgs.length}</span>
            ...
            <div className="relative flex items-center justify-center rounded-xl bg-stone-100">
              ...
              {imgs.length > 1 && (
                <>
                  <button type="button" onClick={() => setPrevI(p => (p ?? 0) === 0 ? imgs.length - 1 : (p ?? 0) - 1)} className="absolute left-3 rounded-full bg-white/90 p-2"><ChevronLeft className="h-4 w-4" /></button>
                  <button type="button" onClick={() => setPrevI(p => ((p ?? 0) + 1) % imgs.length)} className="absolute right-3 rounded-full bg-white/90 p-2"><ChevronRight className="h-4 w-4" /></button>
                </>
              )}
            </div>
