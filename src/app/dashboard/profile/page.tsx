...
export default function DashboardProfilePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(tp.profileUpdated);
      setMsgOk(true);
    } catch (err: unknown) {
      setMessage(err instanceof Error ? err.message : tp.updateFailed);
      setMsgOk(false);
    } finally {
      setSaving(false);
    }
  ...
  if (loading) {
    return <div className="py-20 text-center text-stone-400">{tp.loading}</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-center gap-2 text-sm text-stone-500">
        ...
        <span className="font-medium text-stone-700">{tp.profile}</span>
      </div>

      <div className="mb-6">
        <h1 className="text-[2rem] font-bold tracking-tight text-stone-900">{tp.profile}</h1>
      ...
      <form onSubmit={handleSave} className="space-y-5">
        <section className="rounded-2xl border border-stone-200 bg-white p-5 shadow-[0_1px_2px_rgba(28,25,23,0.04)] md:p-6">
          <h2 className="text-xl font-semibold text-stone-900">{tp.profilePhoto}</h2>
          <div className="mt-5 flex items-center gap-4 rounded-xl border border-stone-100 bg-stone-50/60 px-4 py-4">
            ...
            <div>
              <div className="text-sm font-semibold text-stone-800">{tp.clickAvatar}</div>
              <div className="mt-1 text-xs text-stone-500">{tp.avatarHint}</div>
            </div>
          </div>
        </section>

        <section className="rounded-2xl border border-stone-200 bg-white p-5 shadow-[0_1px_2px_rgba(28,25,23,0.04)] md:p-6">
          <h2 className="text-xl font-semibold text-stone-900">{tp.personalInfo}</h2>

          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <label className="mb-1.5 block text-sm font-medium text-stone-700">
                {tp.fullName} <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={fullName}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-stone-700">
                {tp.phoneWhatsapp} <span className="text-red-500">*</span>
              </label>
              <input
                type="tel"
                value={phone}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-stone-700">
                {tp.email} <span className="text-red-500">*</span>
              </label>
              <input
                type="email"
                value={email}
            ...
            <div className="md:col-span-2">
              <label className="mb-1.5 block text-sm font-medium text-stone-700">{tp.city}</label>
              <input
                type="text"
                value={city}
        ...
        {message && (
          <p className={`rounded-xl px-4 py-3 text-sm ${msgOk ? 'border border-green-200 bg-green-50 text-green-700' : 'border border-red-200 bg-red-50 text-red-600'}`}>
            {message}
          </p>
        )}
