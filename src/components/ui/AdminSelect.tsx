...
interface SelectOption {
  value: string;
  label: string;
  /** Optional group key — options with the same group are rendered under a shared header */
  group?: string;
}
...
interface AdminSelectProps {
  ...
  size?: 'sm' | 'lg';
  /** Show a search box at the top. Recommended for lists > 10 items. */
  searchable?: boolean;
}
  ...
  ({ value, onChange, options, className = '', disabled, error, size = 'lg', searchable = false }, ref) => {
    ...
    function renderOptions(opts: SelectOption[], itemClass: string) {
      if (!hasGroups) {
        return opts.map((opt) => (
          <button
            ...
            className={`w-full text-left transition hover:bg-stone-50 ${itemClass} ${
              opt.value === value
      ...
      return (
        <>
          {groups.map((grp) => {
            const members = opts.filter((o) => o.group === grp);
            return (
              <div key={grp}>
                <div className="px-5 pt-3 pb-1 text-[10px] font-bold tracking-widest uppercase text-[#b8864a]">
                  {grp}
                </div>
                {members.map((opt) => (
                  <button
