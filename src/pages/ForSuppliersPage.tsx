...
// Supplier-specific verification poller
...
function SupplierAuthCard() {
  ...
  useEffect(() => {
    if (localStorage.getItem('supplier_token')) navigate('/supplier/dashboard', { replace: true });
  }, [navigate]);
  ...
  const performRegister = async () => {
    ...
    try {
      ...
      // Try auto-login (will fail if email not yet verified)
      try {
      ...
      } catch { /* expected — email verification required */ }
    ...
    <AuthCardShell className="mx-auto lg:mx-0 order-2 lg:order-1">
      ...
      {step === 'initial' && (
        <div className="space-y-4">
          <a
            href={`${API_BASE}/supplier/auth/google`}
...
export default function ForSuppliersPage() {
  return (
    <div>
      <Helmet>
        <title>List Your Showroom on Tarmeer — Supplier Portal</title>
        <meta name="description" content="List your furniture, stone, lighting or materials showroom on Tarmeer and get discovered by interior designers across UAE." />
        <meta property="og:title" content="Tarmeer Supplier Portal — List Your Showroom" />
        <meta property="og:description" content="Reach UAE renovation companies and interior designers. Upload products, catalogs, and projects." />
