...
export default function AdminSupplierDetailPage() {
  ...
  const saveCatalogTitle = async (catalogId: number) => {
    ...
    try {
      await adminApi.request(`/suppliers/catalogs/${catalogId}/title`, {
        method: 'PATCH',
      ...
      showToast(t('Failed to rename', '重命名失败'), 'error');
    } finally {
                      ...
                      <div className="flex-1 min-w-0">
                        {isEditing ? (
                          <input
                            ...
                            onKeyDown={e => {
                              if (e.key === 'Enter') saveCatalogTitle(c.id);
                              if (e.key === 'Escape') cancelEditCatalog();
                            }}
                            disabled={isSaving}
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          <button
                            ...
                            disabled={isSaving}
                            className="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 transition disabled:opacity-50"
                            title={t('Cancel', '取消')}
                          >
                            <X className="w-4 h-4" />
                          </button>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded-lg text-stone-300 hover:text-[#b8864a] hover:bg-stone-100 transition opacity-0 group-hover:opacity-100"
                            title={t('Rename', '重命名')}
                          >
