/**
 * AdminSignedCompaniesPage — 已签约装企列表（is_signed=1，合并 company_profiles + uae_companies 两表）。
 */
...
export default function AdminSignedCompaniesPage() {
  ...
  const goDetail = (r: Row) => {
    // 把"从哪里来"塞进 history.state，详情页 back 按钮优先读这个
    const fromState = { from: '/admin/signed-companies', fromLabel: '已签约装企' };
    if (r.source === 'profile') navigate(`/admin/profile-companies/${r.id}`, { state: fromState });
    else navigate(`/admin/companies/${r.id}`, { state: fromState });
  };
