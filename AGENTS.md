...
### 第六步之二：三轮代码审查（强制门禁，不得跳过）

**铁律：用例自测全绿 ≠ 代码质量过关。凡改产品源码（`src/`、`server/`）或要交付/上线的代码，声称"完成"前必须过三轮代码审查。每轮用独立子代理（`superpowers:code-reviewer` 或 `/code-review`）对本次 diff 做审查，发现问题→修→复审，直到该轮清白才进下一轮。**

> 起因（2026-07-07）：GEO schema 改动只跑了 tsc/build 就声称完成，实则埋了 4 个会上线即坏/违反隔离铁律的缺陷（whatsapp 电话号拼成 `https://+971…`、`@id` 硬编码 AE 域泄进 VN 站、logo 相对路径拼成 `https:///uploads`、`</script>` 注入）。单轮自检抓不出，故立此三轮门禁。

| 轮次 | 视角 | 必查 |
|------|------|------|
| **第 1 轮 · 规格+安全对抗** | 做没做对 spec（不多不少）+ 攻击面 | 国家隔离泄漏(AE↔VN↔SA 串域/串号)、注入(`dangerouslySetInnerHTML`+用户自填文本必须转义 `<`)、拼 URL 垃圾(电话号/相对路径直接拼协议)、null/边界、软 404 |
| **第 2 轮 · 复审+质量** | 上一轮的修复真的对吗 + 代码质量 | 复核第1轮所有修复无回归；命名/DRY/复用既有 helper(如 `resolveImageUrl`)/死代码/魔法值 |
| **第 3 轮 · 整体+遗漏** | 整体一致性 + 还漏了什么 | 集成一致(多处同一实体 `@id` 是否统一)、真实渲染/构建验证、before/after 是否成立、"哪个改动没被验证" |

规则：
- **每轮发现问题就地修，同一轮复审到清白**，再进下一轮；不得带着未决问题跳轮。
- **产品源码改动三轮全走**；纯内部脚本/文档改动至少走第 1 轮。
- **报告里必须附三轮结论**（各轮发现数 + 修复项 + 最终清白）。
- 三轮未过 = 不能声称"完成"、不能进第七步、不能部署。
