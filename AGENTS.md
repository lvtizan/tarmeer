...
- Never deploy without running tests first. Always run `tsc` in the correct project directory and verify the build passes before any deployment.
...
- Do not make excessive or unauthorized changes. When fixing a bug, change only what's necessary. Never remove or overwrite existing working functionality without explicit user approval.
...
- For git operations: always check for merge conflicts after pull, resolve them carefully, and never delete or overwrite files in dist/ or images/ without explicit permission.
...
- When extracting structured data from Chinese text passages, ALWAYS use the required output tool/format specified. Never fabricate events not present in the source text. Stick strictly to facts in the passage.

## Project-Specific Conventions (Tarmeer)

- When the user says 'go to my backend' they mean the company dashboard, not the admin panel. 'Interview page' means the mobile form, not the admin page. Always ask for clarification if a navigation request is ambiguous.
...
12. **改完代码后的自动流程**：Stop hook 会在每次 Codex 停止时自动运行 tsc 检查（仅当本次 session 修改了 src/ 或 server/src/ 中的 TS/TSX/JS 文件时触发）。tsc 通过后，Codex 必须：（1）运行相关 harness 测试用例；（2）提供本地测试地址（`http://localhost:5173/` 对应路径）；（3）告知用户改了什么 + 测试结果；（4）等用户确认后才能部署。**tsc 失败时 Codex 会被自动唤醒修复，不需要用户介入。**
