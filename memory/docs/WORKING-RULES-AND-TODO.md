...
12. 前端上线前，必须先阅读 `docs/operations/deploy-safety-workflow.md`，并按其中清单执行。
13. 启动本地联调前，必须先完成本文件“启动前 MD 检查清单”。
...
每次开始本地测试登录/注册/上传前，按顺序检查：

1. 后端先启动：`cd server && npm run dev`
2. 确认后端监听 3002：`lsof -iTCP:3002 -sTCP:LISTEN -nP`
3. 前端再启动：`npm run dev -- --host 127.0.0.1 --port 5173`
4. 代理放行本地直连：`localhost,127.0.0.1` 不走代理（避免请求被转发到 `127.0.0.1:1082`）
5. 健康检查：浏览器打开 `http://127.0.0.1:3002/api/health`，必须返回 JSON
6. 前端验收入口：
   - 登录页：`http://127.0.0.1:5173/auth`
   - 设计师列表：`http://127.0.0.1:5173/designers`
   - 设计师个人页：`http://127.0.0.1:5173/designers/salma-nasser`
...
### 2026-03-17 前端部署安全规则固化

- 已新增部署必读文档：
  - `docs/operations/deploy-safety-workflow.md`
- 已将部署前阅读规则写入本文件固定协作规则。
- 已给 `deploy-simple.sh` 增加强制门禁：
  - 未设置 `DEPLOY_RULES_ACK=YES` 会直接拒绝部署并打印规则摘要。
  - 正确执行方式：`DEPLOY_RULES_ACK=YES bash deploy-simple.sh`
